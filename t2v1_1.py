...
currentTime = dt.datetime.now()+dt.timedelta(minutes = 45)
trainsToB=[]
trainsToA=[]
trainsToBstring=[]
trainsToAstring=[]
...
# startTime=dt.time(hour=10)
# endTime=startTime+dt.timedelta(minutes=60) #time between source and destination = 60 min
# print startTimestartTime=dt.time(hour=10)
# endTime=startTime+dt.timedelta(minutes=60) #time between source and destination = 60 min
# print startTime
# Timetakens

def timetable(numOfStations):
    i=0
    
    sameTrainStop = currentTime
    returnTrainStop = currentTime + dt.timedelta(minutes=5)
    ...
    for i in xrange(5):
        ...
        for j in xrange(numOfStations):
            trainsToB[i].append(sameTrainStop)
            trainsToA[i].append(returnTrainStop)
            trainsToBstring[i].append(dt.datetime.strftime(sameTrainStop, frmt))
            trainsToAstring[i].append(dt.datetime.strftime(returnTrainStop, frmt))
            sameTrainStop+=dt.timedelta(minutes=6)
            returnTrainStop+=dt.timedelta(minutes=6)
        x=sameTrainStop-dt.timedelta(minutes=6)
        y=returnTrainStop-dt.timedelta(minutes=6)
        sameTrainStop=dt.timedelta(minutes=5)+y
        returnTrainStop=dt.timedelta(minutes=5)+x
    else:
        print "dead"
        

    print np.matrix(trainsToB)
    print "\n\n"
    print np.matrix(trainsToA)

timetable(6)


# def createTraintoB(numOfStations):
#     numberOfTrains = numOfStations // 3
#     trainStopAtStation = currentTime
...
#         trainsToBstring.append([])
#         for j in xrange(numOfStations):
#             trainsToB[i].append(trainStopAtStation)
#             trainsToBstring[i].append(dt.datetime.strftime(trainStopAtStation, frmt))
#             trainStopAtStation+=dt.timedelta(minutes = 6)

#         NextTrainTime+=dt.timedelta(minutes = (6*2*numOfStations)+10)
#         trainStopAtStation = NextTrainTime
...
# def createTraintoA(numOfStations):
#     numberOfTrains = numOfStations // 3
#     trainStopAtStation = currentTime + dt.timedelta(minutes=5)
#     NextTrainTime = currentTime
#     # trainTimeTable = 
    
#     flat_list_TrainTimetableStringtime = []
#     for i in xrange(5):
#         trainsToA.append([])
#         trainsToAstring.append([])
#         for j in xrange(numOfStations):
#             trainsToA[i].append(trainStopAtStation)
#             trainsToAstring[i].append(dt.datetime.strftime(trainStopAtStation, frmt))
#             trainStopAtStation+=dt.timedelta(minutes = 6)
#         NextTrainTime+=dt.timedelta(minutes = (6*2*numOfStations)+10)
#         trainStopAtStation = NextTrainTime
